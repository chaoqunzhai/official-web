<template>
  <div>
    <div class="n-layout-page-header">
      <n-card :bordered="false" title="分类">
    ...
    <n-grid class="mt-4" cols="1 s:1 m:1 l:4 xl:4 2xl:4" responsive="screen" :x-gap="12">
      <n-gi span="1">
        <n-card :segmented="{ content: true }" :bordered="false" size="small">
          <template #header>
            <n-space>
              <n-button type="info" icon-placement="left" @click="addTable" v-if="hasPermission(['/cdn/cdnDeviceCategory/edit'])">
                <template #icon>
              ...
              <n-button v-if="hasPermission(['/cdn/cdnDeviceCategory/edit'])" type="info" icon-placement="left" @click="handleEdit(selectedState)" :disabled="selectedState.id < 1">
                <template #icon>
                  <div class="flex items-center">
                    <n-icon size="14">
                      <EditOutlined />
              ...
              <n-button v-if="hasPermission(['/cdn/cdnDeviceCategory/delete'])" type="error" icon-placement="left" @click="handleEdit(selectedState)" :disabled="selectedState.id < 1">
                <template #icon>
                  <div class="flex items-center">
                    <n-icon size="14">
                      <DeleteOutlined />
          ...
          <div class="w-full menu">
            ...
            <div class="py-3 menu-list">
              ...
              <n-tree v-else show-line block-line cascade virtual-scroll :pattern="pattern" :data="treeOption" :expandedKeys="expandedKeys" style="height: 75vh" key-field="id" label-field="name" @update:selected-keys="handleSelected" @update:expanded-keys="handleOnExpandedKeys" />
            </div>
          </div>
        </n-card>
      </n-gi>
      <n-gi span="3">
        <n-card :bordered="false" class="proCard">
          ...
          <n-result v-show="selectedState.id < 1" status="info" title="提示" description="请先从列表选择一项后，进行编辑">
            <template #footer>
              <n-button type="info" icon-placement="left" @click="handleAdd(selectedState)"  v-if="hasPermission(['/cdn/cdnDeviceCategory/edit'])">
                <template #icon>
                  <div class="flex items-center">
                    <n-icon size="14">
                      <PlusOutlined />
          ...
          <BasicTable v-if="selectedState.id > 0" ref="actionRef" openChecked :columns="columns" :request="loadDataTable" :row-key="(row) => row.id" :actionColumn="actionColumn" :scroll-x="scrollX" :resizeHeightOffset="-10000"  :checked-row-keys="checkedIds" @update:checked-row-keys="handleOnCheckedRow">
            <template #tableTitle>
              <n-button type="primary" @click="handleAdd(selectedState)"  class="min-left-space" v-if="hasPermission(['/cdn/cdnDeviceCategory/edit'])">
                <template #icon>
                  <n-icon>
                    <PlusOutlined />
                  </n-icon>
              ...
              <n-button type="error" @click="handleBatchDelete" class="min-left-space" v-if="hasPermission(['/cdn/cdnDeviceCategory/delete'])">
                <template #icon>
                  <n-icon>
                    <DeleteOutlined />
                  </n-icon>
...
<script lang="ts" setup>
  ...
  import { List, Delete, TreeOption } from '@/api/addons/cdn/cdnDeviceCategory';
      ...
      return h(TableAction as any, {
        ...
        actions: [

          {
            ...
            auth: ['/cdn/cdnDeviceCategory/edit'],
          },

          {
            label: '删除',
            onClick: handleDelete.bind(null, record),
            auth: ['/cdn/cdnDeviceCategory/delete'],
          },
        ],
      });
    },
